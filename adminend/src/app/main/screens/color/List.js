...
function Color(props) {
	...
	const searchText = useSelector(({ app }) => app.colors.searchText);
	...
	useEffect(() => {
		dispatch(getColors()).then(() => setLoading(false));
	}, [dispatch]);

	useEffect(() => {
		if (searchText.length !== 0) {
			setData(_.filter(colors, item => item.title.toLowerCase().includes(searchText.toLowerCase())));
			setPage(0);
		} else {
			setData(colors);
		}
	...
	function handleClick(item) {
		props.history.push(`/admin/color/${item._id}`);
	}
	...
	const columns = [
		...
		{ id: 'colorcode', label: 'COLOR CODE', width: 120 }
	];
...
export default withReducer('app', reducer)(Color);
